%% Statistical analysis of the 'SimpAdEx' model parameters for Cav13
% This worksheet contains all code and further explanations to perform statistical
% analyses of the model parameters for CaV13
...
%% 
% We create new data frame that contains CaV13-data only

ind_WT = [ind_PC_L5_CaV13WT',ind_PC_L3_CaV13WT',ind_PC_FS_CaV13WT',ind_PC_BB_CaV13WT'];
ind_KO = [ind_PC_L5_CaV13KO',ind_PC_L3_CaV13KO',ind_PC_FS_CaV13KO',ind_PC_BB_CaV13KO'];
...
disp(['BB: ' num2str(length(ind_PC_BB_CaV13WT)) ', FS: ' num2str(length(ind_PC_FS_CaV13WT)) ', L3-PC: ' num2str(length(ind_PC_L3_CaV13WT)) ', L5-PC: ' num2str(length(ind_PC_L5_CaV13WT))])
...
disp(['BB: ' num2str(length(ind_PC_BB_CaV13KO)) ', FS: ' num2str(length(ind_PC_FS_CaV13KO)) ', L3-PC: ' num2str(length(ind_PC_L3_CaV13KO)) ', L5-PC: ' num2str(length(ind_PC_L5_CaV13KO))])
...
%% 
% Unfortunately, the low number of BB and FS cells does not allow for
% further analyses. Therefor we will focus on L3 and L5 pyramidal cells

Data_WT_L3 = Data(:,[2:5,8:17]);
Data_WT_L3 = cellfun(@(x) x(ind_PC_L3_CaV13WT'), Data_WT_L3, 'UniformOutput', false);
Data_KO_L3 = Data(:,[2:5,8:17]);
Data_KO_L3 = cellfun(@(x) x(ind_PC_L3_CaV13KO'), Data_KO_L3, 'UniformOutput', false);

Data_WT_L5 = Data(:,[2:5,8:17]);
Data_WT_L5 = cellfun(@(x) x(ind_PC_L5_CaV13WT'), Data_WT_L5, 'UniformOutput', false);
Data_KO_L5 = Data(:,[2:5,8:17]);
Data_KO_L5 = cellfun(@(x) x(ind_PC_L5_CaV13KO'), Data_KO_L5, 'UniformOutput', false);

%% Analysis of L3 PYR cells
% We pool data from 'old' and 'very old' animals together and
...
for i=1:10
    subplot(2,5,i)
    EMin = min([Data_WT_L3{1,4+i},Data_KO_L3{1,4+i}]);
    EMax = max([Data_WT_L3{1,4+i},Data_KO_L3{1,4+i}]);
    edges = linspace(EMin,EMax,6);
    histogram(Data_WT_L3{1,4+i},edges,'FaceColor','b','Normalization','pdf')
    hold on;
    histogram(Data_KO_L3{1,4+i},edges,'FaceColor','r','Normalization','pdf')
    [f1,x1] = ksdensity(Data_WT_L3{1,4+i});
    [f2,x2] = ksdensity(Data_KO_L3{1,4+i});
...
for i=1:10
    subplot(2,5,i)
    Val = [Data_WT_L3{1,4+i},Data_KO_L3{1,4+i}];
    Label = [ones(1,length(Data_WT_L3{1,4+i})),2*ones(1,length(Data_KO_L3{1,4+i}))];
...
for i=[1,2,4,5,6,7,9,10]
    x = [x Data_WT_L3{1,4+i}'];
    y = [y Data_KO_L3{1,4+i}'];   
end
...
% We pool data from 'old' and 'very old' animals together and
% generate the model parameter histograms to eyeball the sample
% distributions and to receive a first impression of differences between
% WT and KO.
...
% 
% ind_PC_L3_CaV13KO
% ind_PC_L3_CaV13KO_P45 % = ind_PC_L3_CaV13KO
% ind_PC_L3_CaV13WT
% ind_PC_L3_CaV13WT_P45
% 
% ind_PC_L5_CaV13KO     % contains ages 45 and 70
% ind_PC_L5_CaV13KO_P45
% ind_PC_L5_CaV13KO_P70
% ind_PC_L5_CaV13WT
% ind_PC_L5_CaV13WT_P45 % contains ages 45 and 70
% ind_PC_L5_CaV13WT_P70
